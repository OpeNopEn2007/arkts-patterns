...
if (!skillName || !promptFile) {
  console.error('Usage: node tests/skill-triggering/run-test.mjs <skill-name> <prompt-file> [max-turns]');
  process.exit(1);
}
...
const runRoot = resolve(repoRoot, 'tmp', 'skill-triggering', skillName, Date.now().toString());
